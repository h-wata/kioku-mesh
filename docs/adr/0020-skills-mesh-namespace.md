...
> **Revision 2026-06-12**: ADR-0019 の改訂（tier 名 `priv/team/pub` →
> `user/team/mesh`、local-only tier 廃止）に追随して visibility 節を更新。
> 旧版の「個人の複数 PC 間（priv）」という記述は、旧 priv の定義
> （Zenoh に載せない = 複数 PC 間で共有 *できない*）と矛盾していたが、
> user tier（自分のマシン間で複製）の導入によりこの用途が正しく表現できる
> ようになった。
...
```text
skill/{visibility}/{scope_id}/index/{name}
skill/{visibility}/{scope_id}/body/{name}/{version}

# scope_id: visibility == user なら user_id、team なら team_id（ADR-0019 と同じ解決規則）
```
...
### Visibility — 初期スコープは user / team のみ

ADR-0019 の visibility tier（user / team / mesh）をそのまま採用するが、
**`mesh` は当面封印**する。

```text
skill/user/{user_id}/... → その user の各マシンにのみ複製（ADR-0019 の user と同じ）
skill/team/{team_id}/... → 該当 team storage を持つ peer のみに複製
skill/mesh/...  → 将来。署名・レビュー基盤ができるまで実装しない
```

理由: skill は実行されるため、不特定多数への配布は署名検証と取り込み
レビューの基盤なしには supply-chain 攻撃面になる。個人の複数 PC 間
（user）と、相互に信頼済みの少人数 team（team + mTLS）に限定すれば、
...
- 将来 `mesh` を解禁する場合は、cert チェーンによる署名検証と
...
- 良い点: immutable versioned body は TOCTOU・監査・pin を key 設計だけで
  解決し、上書き競合の調停が不要になる。
- 良い点: user/team 限定により、信頼境界が既存の mTLS 境界と一致する。
  user tier は Zenoh 複製なので「自分の複数 PC で同じ skill セットを使う」
  という本来の動機（Context 冒頭）がそのまま実現できる。
...
- 悪い点: `mesh` 封印により、OSS 的な skill 配布のユースケースは当面
  対象外になる。
