...
- Status: Accepted
- Date: 2026-06-10 (Accepted: 2026-06-25)
- Supersedes: なし（ADR-0007 の「今後: FTS5 は要件が見えたら段階導入」を具体化）
- Related: ADR-0002, ADR-0007, ADR-0010, ADR-0011, Issue #167, Issue #203 (実装)
...
- 旧バージョンへのロールバック時は未知テーブル / 列が無視されるだけで
  動作する（SQLite は additive change に対して双方向に寛容）。ただし
  FTS 非対応の旧コードが `obs_index` を更新すると `obs_fts` が同期から
  外れる。実装では connect 時に `obs_fts` と `obs_index` の行数を比較し、
  不一致なら `obs_fts` を full backfill する resync ガードを入れ、
  downgrade→upgrade で行が検索に出ないという静かな壊れ方を防ぐ
  （完全な行単位 reconcile は `rebuild_from_zenoh` が担保）。
