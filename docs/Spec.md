...
この文書は、2026-06-01 時点のリポジトリ実装から読み取れる現状仕様をまとめたものです。将来構想ではなく、`src/mesh_mem/` と `tests/` が現在保証している振る舞いを基準にしています。

関連図:

- draw.io 編集元: [kioku-mesh-state-transitions.drawio](./kioku-mesh-state-transitions.drawio)

### Observation lifecycle

![Observation lifecycle](./kioku-mesh-observation-lifecycle.svg)

### SQLite sidecar index sync

![SQLite sidecar index sync](./kioku-mesh-index-sync.svg)

## 1. 目的と前提

kioku-mesh は、複数の AI コーディングエージェントと複数 PC が作業上の観測・判断・不具合原因・設定変更などを共有するための分散メモリです。

- `local` backend は SQLite のみで動作し、zenohd を必要としない単一ホスト永続モード。
- `zenoh` backend は Zenoh 1.9 系を前提とし、永続化の真実は zenohd + RocksDB backend 上の key-value にある。
- `zenoh` backend での Python 側 SQLite は検索高速化のためのローカルサイドカーであり、正本ではない。
- MCP サーバは stdio transport を前提とし、Claude Code / Claude Desktop / Gemini CLI / Codex CLI などの MCP ホストから呼び出される。
- パッケージの公開名は `kioku-mesh`、CLI は `kioku-mesh`、MCP サーバは `kioku-mesh-mcp`。
...
`mesh_mem.backend.get_backend()` が `LocalBackend` または `ZenohBackend` を返し、CLI と MCP は同じ backend surface を通ります。
...
- `delete [OBSERVATION_ID]`
  - `--project`, `--pc-id`, `--since`, `--until`, `--dry-run`, `--yes`, `--batch-size`, `--reason`
- `status`
  - backend mode、version、`pc_id`、`session_id`、最大 `MAX_SEARCH` 件内の件数、family/pc 別件数、Zenoh status を表示。
- `drain`
  - `--pending`, `--limit`
- `gc`
...
- `doctor`
  - `--json`
- `mcp install`
  - `--client {claude-code,codex-cli}`, `--name`, `--env`, `--force`, `--dry-run`
- `mesh start` / `mesh join`
  - zenohd binary なしの try-it / demo 用 ephemeral mesh。
- `tls init-ca` / `tls request` / `tls sign` / `tls install` / `tls enroll` / `tls info`
  - private CA と peer cert を管理し、`tls enroll` は request → sign over SSH → install をまとめる wrapper。
