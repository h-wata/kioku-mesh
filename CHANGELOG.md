...
- **README adds an "Install zenohd" section** (#83) before the Quick start, with copy-pasteable apt (Eclipse Debian repo) and prebuilt-zip recipes, a from-source cargo fallback (with the toolchain-match caveat from upstream), and a verify block covering `zenohd --version` plus the rocksdb backend log line. The existing `## Requirements` Zenoh 1.9 bullet now cross-links to it so first-touch users no longer have to read upstream Zenoh docs to get `zenohd` on `PATH`.
