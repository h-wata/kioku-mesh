...
- **`mesh-mem demo` subcommand** (#108): seeds 3 example observations (decision / bug / pattern) via the local backend with no zenohd required. Initializes the local backend idempotently, prints "try this now" hints for `mesh-mem search`, and bridges to `mesh-mem mcp install`. Demo seed path and MCP agent save path share the same `get_backend().put_observation()` surface.
