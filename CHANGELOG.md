...
- ruff: PLR0912/PLR0915 (分岐過多・文過多) を有効化 (max-branches=20, max-statements=80。
  cross-review 指摘 B1 を受け、当初案の 25/120 では新規 100 statements 級の関数が素通り
  するため強化した)。既存違反は `# noqa: PLR0912` / `# noqa: PLR0915` を関数単位で個別付与
  して抑制し (ファイル単位の一括抑制はしない)、同じファイルでの新規違反は引き続き検出
  される。詳細: `queue/projects/kioku-mesh/reports/details/TASK-469_plr_violations.md`,
  `queue/projects/kioku-mesh/reports/details/TASK-488_plr_threshold.md`
- 長寿命 MCP server の定期 index 再アラインメント worker。`kioku-mesh-mcp` が
  ...
  失敗も記録しない)。停止確認は selector の開始前・reply ごと・selector 完了後に行うので、
  返信ゼロで終わる selector が停止をすり抜けて partial scan を成功として記録することはない。
  応答の来ない scan で待ち時間を超えた場合は、停止できなかったことを stderr に警告し、
  生存中の worker の session.get と並走しないよう backend の teardown を省いて
  process 終了に委ねる (#325, ADR-0035, TASK-450 Phase 2)
...
- `LocalIndex.repair_from_zenoh(mode=...)`: zenoh で見えた observation / tombstone
  ...
  `rebuild_from_zenoh` の shadow semantics は変更していない。full mode の repair は
  R5 (#326) の orphan tombstone placeholder も rebuild 経路と共通の helper
