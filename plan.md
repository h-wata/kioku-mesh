...
│   ↓ MCP (stdio)        │            │   ↓ MCP (stdio)        │
│ zenoh-mem MCP server   │            │ zenoh-mem MCP server   │
...
| 識別子 | 責務 | 値 | 解決方法 |
|---|---|---|---|
| `agent_family` | エージェント系統 | `claude` / `gemini` / `codex` / `chatgpt` | env `ZENOH_MEM_AGENT_FAMILY` |
| `client_id` | 実クライアント種別 | `claude-code` / `claude-desktop` / `gemini-cli` / `codex-cli` / `chatgpt-desktop` | env `ZENOH_MEM_CLIENT_ID` |
| `pc_id` | PC固有の安定ID（UUID） | 初回生成して永続化 | `$ZENOH_MEM_STATE_DIR/pc_id` 読み書き |
| `session_id` | エージェント起動単位 | `{yyyymmddTHHMMSS}-{short}` | env `ZENOH_MEM_SESSION_ID` 優先、無ければ自動生成 |
...
def state_dir() -> pathlib.Path:
    d = pathlib.Path(
        os.environ.get(
            "ZENOH_MEM_STATE_DIR",
            pathlib.Path.home() / ".local/share/zenoh-mem",
        )
    )
...
def get_agent_family() -> str:
    return os.environ.get("ZENOH_MEM_AGENT_FAMILY", "unknown")


def get_client_id() -> str:
    return os.environ.get("ZENOH_MEM_CLIENT_ID", "unknown")


def get_session_id() -> str:
    """プロセス起動時に 1 回だけ確定してキャッシュ。
    - env ZENOH_MEM_SESSION_ID があればそれ
    - 無ければ `{yyyymmddTHHMMSSZ}-{short}` を自動生成
    """
    global _session_id_cache
    if _session_id_cache is not None:
        return _session_id_cache
    sid = os.environ.get("ZENOH_MEM_SESSION_ID")
    if not sid:
...
### `src/zenoh_mem/store.py`（要点。完全版は実装時に詰める）
...
### `src/zenoh_mem/mcp_server.py`

```python
"""zenoh-mem MCP Server (FastMCP)"""
...
def delete_memory(observation_id: str, reason: str = "") -> str:
    ...
    observation_id の完全一致（32文字）が必要。物理削除は `zenoh-mem gc` が担当。
...
def _version() -> str:
    try:
        return importlib.metadata.version("zenoh-mem")
    except Exception:
        return "unknown"
...
- `zenoh-mem save "..."` → env から identity を解決して保存（`--project`, `--tags`）
- `zenoh-mem search "..."` → `--agent-family` / `--client-id` / `--pc-id` / `--session-id` / `--project` / `--since` / `--limit`（limit は MAX_SEARCH=10000 に clamp）
- `zenoh-mem delete <observation_id>` → tombstone（32文字完全一致）
- `zenoh-mem gc [--force-id <id>]` → tombstone 対象の物理削除（retention policy に基づく）
- `zenoh-mem status` → 状態表示（MAX_SEARCH 件以内の集計）
...
[project.scripts]
zenoh-mem = "zenoh_mem.__main__:main"
zenoh-mem-mcp = "zenoh_mem.mcp_server:mcp.run"
```
...
- `command` は `python` 直叩きではなく、インストール済みの `zenoh-mem-mcp` 実行ファイルの**絶対パス**を使う（PATH 依存・venv 事故を回避）。
- `env` で identity を必ず注入する。特に `ZENOH_MEM_AGENT_FAMILY` と `ZENOH_MEM_CLIENT_ID` の取り違えが空間汚染に直結する。
...
```json
{
  "mcpServers": {
    "zenoh-mem": {
      "command": "/home/USER/.venv/zenoh-mem/bin/zenoh-mem-mcp",
      "env": {
        "ZENOH_CONNECT": "tcp/localhost:7447",
        "ZENOH_MEM_AGENT_FAMILY": "claude",
        "ZENOH_MEM_CLIENT_ID": "claude-code"
      }
    }
  }
}
...
`ZENOH_MEM_CLIENT_ID` を `claude-desktop` に変更して同様に登録。

### Gemini CLI: `~/.gemini/settings.json`

`ZENOH_MEM_AGENT_FAMILY=gemini`, `ZENOH_MEM_CLIENT_ID=gemini-cli`。

### Codex CLI / ChatGPT Desktop も同パターン。

### session_id の注入（任意）

エージェント起動フックから `ZENOH_MEM_SESSION_ID` を渡せばセッション識別を制御できる。未指定なら起動時自動生成。
...
export ZENOH_BACKEND_ROCKSDB_ROOT="$HOME/.local/share/zenoh-mem"
...
### Step 2: zenoh-mem のインストール

```bash
# 各PCで venv を作ってインストール（PATH 事故回避のため）
python3 -m venv ~/.venv/zenoh-mem
~/.venv/zenoh-mem/bin/pip install -e .
```
...
# 2. PC-A で数件 put
zenoh-mem save "while B is down 1" --project poc --tags offline-test
...
### 保持期間 / 容量管理
- PoC では observation 無期限保持。Tombstone は 30 日経過後に物理削除（`zenoh-mem gc` で手動 or cron）。
- RocksDB が肥大化した場合は、該当 storage を一時停止 → 両PCで該当キーを delete → replication 同期を待って再起動、という手順を README 化する。

### 機微情報混入時の緊急手順
1. 該当 observation に対し `zenoh-mem delete <id>` を即実行（tombstone 伝播）
2. 両PCで `zenoh-mem gc --force-id <id>` を実行し RocksDB から物理削除
