...
# KIOKU_MESH_FORCE_REBUILD=1 and KIOKU_MESH_SKIP_REBUILD=1 override this default,
...
def set_rebuild_on_init_explicit(rebuild: bool | None) -> None:
    ...
    Wins over both ``KIOKU_MESH_FORCE_REBUILD`` / ``KIOKU_MESH_SKIP_REBUILD``
    ...
    that export ``KIOKU_MESH_SKIP_REBUILD=1`` from a shell profile or wrapper
...
def _should_rebuild_on_init() -> bool:
    ...
    Priority (highest to lowest):
      ...
      2. ``KIOKU_MESH_FORCE_REBUILD=1`` env var.
      3. ``KIOKU_MESH_SKIP_REBUILD=1`` env var.
...
def _empty_index_rebuild_allowed() -> bool:
    ...
    *implicit* CLI default-skip (#38); an explicit opt-out
    (``set_rebuild_on_init_explicit(False)`` or ``KIOKU_MESH_SKIP_REBUILD=1``)
