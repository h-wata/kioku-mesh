...
must be patched on kioku_mesh.transport``, not on the ``store`` aliases.
...
def _store() -> ModuleType:
    ...
    factory ``get_session`` consults) live in kioku_mesh.transport`` and
...
# ADR-0023: register pending-count callback with core.transport
# so get_transport_status() can report pending_puts without importing memory.
from kioku_mesh.core import transport as _transport_mod  # noqa: E402

_transport_mod.register_pending_count(_count_pending_puts)
del _transport_mod
