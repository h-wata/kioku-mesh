...
# Once-per-process flags to avoid spamming legacy-related warnings.
...
def get_default_visibility() -> str:
    ...
    Priority (highest to lowest):
      1. ``KIOKU_MESH_DEFAULT_VISIBILITY`` env var
...
def resolve_write_visibility(explicit: str = '') -> tuple[str, str]:
    ...
    if visibility == 'user':
        ...
        if not user_id:
            raise ValueError(
                "visibility 'user' requires a user_id: set KIOKU_MESH_USER_ID or add "
    ...
    if visibility == 'team':
        ...
        if not team_id:
            raise ValueError(
                "visibility 'team' requires a team_id: set KIOKU_MESH_TEAM_ID or add "
