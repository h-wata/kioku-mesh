...
def _render_systemd_unit(config_path: Path, zenohd_binary: str) -> str:
...
def _render_localhost_config(listen_endpoints: list[str]) -> str:
    ...
    return (
        '// Generated by `kioku-mesh init` — single-host (loopback) config.\n'
...
def _render_mesh_config(mode: str, listen_endpoints: list[str], connect_endpoints: list[str]) -> str:
    ...
    return (
        f'// Generated by `kioku-mesh init --mode {mode}` — {role_note}.\n'
...
def _cmd_mesh_start(args: argparse.Namespace) -> int:
    ...
    if _is_wildcard:
        ...
        print(f'  from this host:   ZENOH_CONNECT=tcp/127.0.0.1:{_port} MESH_MEM_BACKEND=zenoh kioku-mesh save ...')
        if _detected:
            ...
            print(
                f'  from other hosts: '
                f'ZENOH_CONNECT=tcp/<this-host-ip>:{_port} MESH_MEM_BACKEND=zenoh kioku-mesh save ...'
            )
            print("                    ^ replace <this-host-ip> with this machine's LAN IP")
    else:
        print(f'Router listening on {args.listen}.')
        print(f'  connect with: ZENOH_CONNECT={args.listen} MESH_MEM_BACKEND=zenoh kioku-mesh save ...')
...
def _cmd_mesh_join(args: argparse.Namespace) -> int:
    ...
    print(f'In another terminal: ZENOH_CONNECT={args.peer} MESH_MEM_BACKEND=zenoh kioku-mesh save ...')
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p_mesh = sub.add_parser(
        ...
        description=(
            ...
            '  Host A or B: ZENOH_CONNECT=tcp/<host-a-ip>:17447 MESH_MEM_BACKEND=zenoh kioku-mesh save "hello mesh"\n'
            '  Host A or B: ZENOH_CONNECT=tcp/<host-a-ip>:17447 MESH_MEM_BACKEND=zenoh kioku-mesh search "hello"\n\n'
            'Env vars:\n'
    ...
    p_mesh_start = p_mesh_sub.add_parser(
        ...
        description=(
            ...
            '  ZENOH_CONNECT=tcp/<this-host>:17447 MESH_MEM_BACKEND=zenoh kioku-mesh save "hello"\n\n'
            'Env vars:\n'
    ...
    p_mesh_join = p_mesh_sub.add_parser(
        ...
        description=(
            ...
            '  ZENOH_CONNECT=tcp/192.168.1.10:17447 MESH_MEM_BACKEND=zenoh kioku-mesh save "hello"\n'
