...
def _detect_systemd_user() -> tuple[bool, str]:
    """Return ``(supported, reason)`` for systemd-user availability on this host.

    The check is intentionally cheap: presence of ``systemctl`` on PATH plus
    a platform gate. We do NOT shell out to verify the user session is alive —
    a host can have the binary without an active user manager, but writing
    the unit file is still meaningful (the user can enable it later from
    a logged-in session).
    """
...
def _render_systemd_unit(config_path: Path, zenohd_binary: str) -> str:
    """Render a systemd --user unit pointing at the given mesh-mem config.
    ...
    not inherit an interactive shell's PATH on every distro.
    """
    return (
        '# Generated by `mesh-mem init --install-systemd`. Safe to edit; re-running\n'
        ...
        f'ExecStart={zenohd_binary} -c {config_path}\n'
        'Restart=on-failure\n'
        'RestartSec=5s\n'
        '\n'
        '[Install]\n'
