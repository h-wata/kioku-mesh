...
Scope publication policy:
  - user:  published when ``user_id`` is configured (MESH_MEM_USER_ID or config.yaml)
  - team:  published when ``team_id`` is configured
  - mesh:  off by default; opt-in via MESH_MEM_MESSAGING_PRESENCE_MESH=1
...
def _publication_scopes() -> list[str]:
    ...
    if os.environ.get('MESH_MEM_MESSAGING_PRESENCE_MESH') == '1':
        scopes.append('mesh')
    return scopes
