...
from kioku_mesh.identity import _sanitize_key_segment
from kioku_mesh.identity import IdentitySource
from kioku_mesh.identity import resolve_agent_family
from kioku_mesh.identity import resolve_client_id
...
def test_env_overrides_auto_generated_session_id(monkeypatch: pytest.MonkeyPatch) -> None:
    identity.reset_caches()
    monkeypatch.setenv('KIOKU_MESH_SESSION_ID', 'explicit-session-001')
...
def test_state_dir_env_override_wins(
    ...
    """KIOKU_MESH_STATE_DIR overrides the per-OS platformdirs default."""
    monkeypatch.setenv('KIOKU_MESH_STATE_DIR', str(tmp_path))
...
def test_state_dir_empty_env_falls_through_to_default(
    ...
    """Empty KIOKU_MESH_STATE_DIR falls through to the per-OS default.

    v0.2.0 treated an empty string as cwd; v0.2.1+ treats it as unset.
    """
    monkeypatch.setenv('KIOKU_MESH_STATE_DIR', '')
...
def test_client_id_env_set(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv('KIOKU_MESH_CLIENT_ID', 'claude-code')
...
def test_client_id_default_is_user_at_host(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv('KIOKU_MESH_CLIENT_ID', raising=False)
    monkeypatch.setenv('USER', 'alice')
    monkeypatch.setattr('kioku_mesh.identity.socket.gethostname', lambda: 'mbp-laptop.local')
    value, source = resolve_client_id()
...
def test_client_id_default_falls_back_when_user_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr('kioku_mesh.identity.getpass.getuser', _raise)
    monkeypatch.setattr('kioku_mesh.identity.socket.gethostname', lambda: '')
    value, source = resolve_client_id()
...
def test_client_id_default_sanitizes_zenoh_unsafe_chars(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr('kioku_mesh.identity.socket.gethostname', lambda: 'host*name?.local')
    value, source = resolve_client_id()
    assert '/' not in value
    assert '*' not in value
    assert '?' not in value
